...
public class HttpDownManager {
     ...
     * @param info
     */
    public void restarDown(DownInfo info) {
        info.setState(DownState.RESTART);
        info.getListener().onRestart();
        if (subMap.containsKey(info.getUrl())) {//不提交数据.暂停
            ProgressDownSubscriber subscriber = subMap.get(info.getUrl());
            subscriber.unsubscribe();
            subMap.remove(info.getUrl());
        }
        //从数据库对应路径然后删除文件,清空进度缓存
        boolean isdeleteFile = deleteFile(info.getSavePath());
        if (isdeleteFile) {
